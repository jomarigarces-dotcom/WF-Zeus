...
let state = {
  ...
  awraUrl:
    "https://script.google.com/a/macros/ececontactcenters.com/s/AKfycbyb-WPlhrGMJH-xaTs8dFCdL2Hd57ny5DslnLeuzH-ZPrytn6-y_m0sIMxyJ999ZZvKdQ/exec",
  userAccounts: [],
  ...
  lastEmojis: JSON.parse(
    localStorage.getItem("zeus_last_emojis") || '["👍", "❤️", "😂"]',
  ),
...
window.showAssignedPopup = function () {
  ...
  state.userAccounts.forEach((acc) => {
    ...
    row.className =
      "flex justify-between items-center my-2 bg-slate-50 dark:bg-slate-800 p-3 rounded-xl border border-slate-100 dark:border-slate-700";
...
window.unassignAccount = function (accId) {
  ...
  (async () => {
    try {
      const res = await runMutation("users:requestAccountRemoval", {
      ...
      Promise.resolve().then(() => {
        ...
        if (typeof _cb === "function") _cb(res);
      });
    } catch (err) {
      console.error(err);
...
window.onload = function () {
  ...
  window.updateActivity = function () {
    ...
    if (s.status !== "OFFLINE" && timeDiff > awayTime) {
      ...
      (async () => {
        try {
          const res = await runMutation("users:updateUserAuxStatus", {
            email: state.userEmail,
            auxStatus: "ONLINE",
          });
        } catch (err) {
...
window.onerror = function (message, source, lineno, colno, error) {
  try {
    ...
    if (window.__zeusErrorHideTimer) {
      clearTimeout(window.__zeusErrorHideTimer);
      window.__zeusErrorHideTimer = null;
    }
    window.__zeusErrorHideTimer = setTimeout(() => {
      try {
        ...
        console.error("Error hiding debug overlay", e);
      }
    }, 8000);
  } catch (e) {
    console.error("Error rendering debug overlay", e);
  }
  return false;
};
...
function handleGlobalClick(event) {
  ...
  if (state.notesOpen) {
    ...
    const isModalOpen =
      (nm && !nm.classList.contains("hidden")) ||
      (nd && !nd.classList.contains("hidden")) ||
      (ne && !ne.classList.contains("hidden"));
    if (
      ns &&
      nt &&
      !ns.contains(event.target) &&
      !nt.contains(event.target) &&
      !isModalOpen
    ) {
      window.toggleNotes();
    }
  ...
  if (
    sb &&
    st &&
    sb.classList.contains("left-0") &&
    !sb.contains(event.target) &&
    !st.contains(event.target)
  )
...
window.pollPersonalUpdates = function (onComplete) {
  ...
  (async () => {
    try {
      const res = await runQuery("tasks:fetchPersonalUpdates", {
        email: state.userEmail,
      });
      Promise.resolve().then(() => {
        const _cb = (data) => {
          ...
          if (
            ...
            data.notifications.forEach((n) => {
              if (n.type === "access") {
                const typeLabel =
                  n.requestType === "REMOVAL" ? "removal" : "access";
                if (n.approved) {
                  const accName =
                    state.availableAccounts.find((a) => a.id === n.accountId)
                      ?.name || n.accountId;
                  if (
                    n.requestType === "REMOVAL" &&
                    state.currentAccount &&
                    state.currentAccount.id === n.accountId
                  ) {
                    alert(
                      `Your removal request for workspace ${accName} has been approved.`,
                    );
                    google.script.run
                    ...
                    alert(
                      `Your access request for workspace ${accName} has been approved.`,
                    );
                  ...
                  const accName =
                    state.availableAccounts.find((a) => a.id === n.accountId)
                      ?.name || n.accountId;
                  alert(
                    `Your ${typeLabel} request for workspace ${accName} was denied.`,
                  );
                }
              }
            });
...
window.pollWorkspaceUpdates = function (onComplete) {
  if (!state.userEmail || !state.currentAccount) {
    if (onComplete) onComplete();
    return;
  }
  (async () => {
    try {
      const res = await runQuery("accounts:getAccountData", {
    ...
    const cards = reminders
      .map((r) => {
        ...
        const deleteBtn = canDelete
          ? `<span class="material-icons text-lg text-slate-300 hover:text-red-500 cursor-pointer transition-colors" onclick="deleteReminder('${r.id}')">delete</span>`
          : "";
        // Avatar fallback
...
window.updateChecklistUI = function () {
  ...
  const filteredTasks = state.checklist
    .filter((t) => (isPendingView ? !t.isDone : t.isDone))
...
window.renderTaskQueue = function () {
  ...
  if (state.bulkTaskQueue.length === 0) {
    c.innerHTML =
      '<div class="flex h-full items-center justify-center text-slate-400 text-xs italic">Queue empty</div>';
    btn.disabled = true;
  } else {
    c.innerHTML = state.bulkTaskQueue
      .map(
        (t, idx) => `
...
window.dispatchBulkTasks = function () {
  ...
  (async () => {
    try {
      const res = await runMutation("tasks:bulkAssignTasks", {
        callerEmail: state.userEmail,
        targetType: state.bulkTargetType,
        targetId: targetId,
        tasks: state.bulkTaskQueue,
        senderNickname: state.userNickname,
      });
      Promise.resolve().then(() => {
        const _cb = (msg) => {
          alert(msg);
...
window.fetchLiveStatus = function () {
  (async () => {
    try {
      const res = await runQuery("status:getLiveStatus", {});
      Promise.resolve().then(() => {
        const _cb = renderLiveStatus;
        if (typeof _cb === "function") _cb(res);
      });
    } catch (err) {
      console.error(err);
...
window.renderLiveStatus = function (users) {
  ...
  tbody.innerHTML =
    html ||
    '<tr><td colspan="3" class="p-4 text-center text-slate-400 text-xs italic">No active users found.</td></tr>';
...
window.fetchTaskHistory = function () {
  ...
  document.getElementById("history-table-body").innerHTML =
    '<tr><td colspan="4" class="p-8 text-center"><span class="material-icons animate-spin text-slate-400">sync</span></td></tr>';
  (async () => {
    try {
      const res = await runQuery("tasks:getTaskHistory", {
        callerEmail: state.userEmail,
        filterAccount: acc,
        filterUser: usr,
        dateFrom: dFrom,
...
window.renderTaskHistory = function (tasks) {
  ...
  if (tasks.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="4" class="p-8 text-center text-xs text-slate-400 font-bold uppercase tracking-widest">No history found</td></tr>';
    return;
  }
  tbody.innerHTML = tasks
    .map((t) => {
      const completedDate =
        t.completedAt && t.completedAt !== "Unknown"
...
window.openFeedback = function () {
  const isJomz =
    state.userEmail === "jomari.garces@ececontactcenters.com" ||
...
window.sendFeedback = function () {
  ...
  (async () => {
    try {
      const res = await runMutation("feedback:submitFeedback", {
        email: state.userEmail,
...
window.loadFeedbacks = function () {
  ...
  list.innerHTML =
    '<div class="text-center text-slate-400 text-xs py-10"><span class="material-icons animate-spin">sync</span></div>';
  (async () => {
    try {
      const res = await runQuery("feedback:getFeedbacks", {});
      Promise.resolve().then(() => {
        const _cb = (feedbacks) => {
          if (feedbacks.length === 0) {
            list.innerHTML =
              '<div class="text-center text-slate-400 text-xs py-10 font-bold uppercase tracking-widest">No feedbacks yet</div>';
            return;
          }
...
function initSession() {
  (async () => {
    try {
      const res = await runQuery("users:getSessionInfo", {});
      Promise.resolve().then(() => {
        const _cb = function (info) {
          state.role = info.role;
          ...
          if (info.role === "SUPER_ADMIN") {
            ...
            if (
              lastAccountId &&
              info.accounts.some((a) => a.id === lastAccountId)
            ) {
              window.loadAccount(lastAccountId);
            } else {
            ...
            if (
              ...
              window.loadAccount(lastAccountId);
            } else if (info.assignedAccount) {
              window.loadAccount(info.assignedAccount);
            } else {
              showRegistration();
            }
...
window.toggleMaintenanceMode = function () {
  ...
  if (
    !confirm(
      `Are you sure you want to ${action} Maintenance Mode? This will affect all users.`,
    )
  )
    return;
  ...
  (async () => {
    try {
      const res = await runMutation("users:toggleMaintenanceMode", {});
      Promise.resolve().then(() => {
        const _cb = (status) => {
...
function renderMaintenanceUI() {
  ...
  if (state.canToggleMaintenance) {
    if (toggleBtn) {
      ...
      toggleBtn.innerText = state.isMaintenanceMode
        ? "OFF MAINT."
        : "MAINTENANCE";
...
window.loadAccount = async function (accountId) {
  ...
  const hasAccess =
    state.role === "SUPER_ADMIN" ||
    state.userAccounts.some((a) => a.id === accountId);
  if (!hasAccess) {
  ...
  grid.innerHTML =
    '<div class="flex items-center justify-center min-h-[50vh]"><span class="material-icons text-4xl text-primary-600 animate-spin">sync</span></div>';
  localStorage.setItem("zeus_last_account", accountId);
  (async () => {
    try {
      const res = await runMutation("unknown:getAccountData", {
        args: [accountId],
      });
      Promise.resolve().then(() => {
        const _cb = function (data) {
          ...
          document
            .getElementById("admin-registry-header")
          ...
          document.getElementById("manageBtn").className =
            "bg-primary-600 hover:bg-primary-700 text-white text-[10px] font-black tracking-widest px-4 py-2 rounded-full transition-all uppercase shadow-md shadow-primary-500/20";
          ...
          document
            .getElementById("orgChartBtn")
            .classList.toggle("hidden", !isWalmart);
          renderCategories();
          renderContent();
        };
        if (typeof _cb === "function") _cb(res);
  ...
  if (state.currentCat === "HOME") {
    ...
    if (!homeLayout) {
      ...
      homeLayout.className =
        "flex flex-col gap-4 w-full h-full overflow-hidden pb-4";
      ...
      heroSection.className = "w-full h-[120px] shrink-0";
      ...
      splitContent.className = "flex gap-6 flex-1 min-h-0";
      const announcementsCol = document.createElement("div");
      announcementsCol.className =
        "w-[28%] flex flex-col h-full overflow-hidden";
      ...
      rightCol.className = "flex-1 flex flex-col gap-4 h-full overflow-hidden";
      ...
      bottomRow.className = "flex gap-6 flex-1 min-h-0";
      ...
      checklistCol.className =
        "w-[35%] flex flex-col h-full overflow-hidden min-w-[300px]";
      checklistCol.id = "checklist-wrapper";
      bottomRow.appendChild(remindersCol);
      bottomRow.appendChild(checklistCol);
      rightCol.appendChild(bottomRow);
      splitContent.appendChild(announcementsCol);
      splitContent.appendChild(rightCol);
      homeLayout.appendChild(heroSection);
      homeLayout.appendChild(splitContent);
    ...
    main.classList.remove("overflow-hidden");
    const filteredIcons = state.currentAccount.icons.filter(
      (i) => i.catId == state.currentCat,
    );
...
window.updateWeatherUI = function () {
  ...
  if (!state.weatherRisk) {
    ...
    const impacts =
      w.impacts
        .map(
          (i) =>
            `<div class="flex justify-between items-center text-[9px] border-b border-slate-100 dark:border-slate-800 pb-1"><span class="font-bold text-slate-600 dark:text-slate-300">${new Date(i.time).toLocaleTimeString([], { hour: "numeric", hour12: true, timeZone: state.systemTimeZone })}</span><span class="font-black ${i.rain > 1.5 ? "text-red-500" : "text-slate-400"}">${i.rain}mm rain</span></div>`,
        )
        .join("") ||
      '<div class="text-[9px] text-slate-400 italic">No significant peak impacts</div>';
  ...
  if (weatherSection.innerHTML !== newHTML) {
    weatherSection.innerHTML = newHTML;
  }
};
window.fetchRegistry = function () {
  ...
  (async () => {
    try {
      const res = await runMutation("unknown:getUsersRegistry", {});
      Promise.resolve().then(() => {
        const _cb = function (registry) {
...
window.renderAdminRegistry = function (registry) {
  ...
  for (const id in registry) {
    ...
    ad.className =
      "px-4 py-2 mt-2 text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-tight flex items-center gap-2";
    ...
    if (a.users.length === 0) {
      ...
      a.users.forEach((u) => {
        ...
        ud.className =
          "group flex items-center justify-between px-8 py-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-default";
        ud.innerHTML = `
          <div class="flex flex-col">
...
window.unregisterAction = function (aid, email) {
  if (confirm(`Remove ${email}?`)) {
    (async () => {
      try {
        const res = await runMutation("accounts:unregisterUser", {
          callerEmail: state.userEmail,
          accountId: aid,
          email: email.trim().toLowerCase(),
        });
        Promise.resolve().then(() => {
          const _cb = (r) => renderAdminRegistry(r);
          if (typeof _cb === "function") _cb(res);
        });
      } catch (err) {
        console.error(err);
...
window.fetchAccessRequests = function () {
  (async () => {
    try {
      const res = await runQuery("users:getAccessRequests", {});
      Promise.resolve().then(() => {
        const _cb = renderAccessRequests;
        if (typeof _cb === "function") _cb(res);
      });
    } catch (err) {
      console.error(err);
...
window.renderAccessRequests = function (reqs) {
  ...
  if (btn)
    btn.innerText = `Requests${reqs && reqs.length ? " (" + reqs.length + ")" : ""}`;
  if (!c) return;
  if (!reqs || reqs.length === 0) {
    c.innerHTML =
      '<div class="text-center text-xs text-slate-400">No pending requests.</div>';
    return;
  }
  c.innerHTML = reqs
    .map(
...
window.approveRequest = function (requestId) {
  (async () => {
    try {
      const res = await runMutation("users:approveAccountAccess", {
        callerEmail: state.userEmail,
        requestId: requestId,
      });
      Promise.resolve().then(() => {
...
window.rejectRequest = function (requestId) {
  (async () => {
    try {
      const res = await runMutation("users:rejectAccountAccess", {
        callerEmail: state.userEmail,
        requestId: requestId,
      });
      Promise.resolve().then(() => {
...
window.toggleRegistrationSelection = function (id) {
  if (state.selectedRegistrationAccounts.has(id))
    state.selectedRegistrationAccounts.delete(id);
...
window.submitRegistration = function () {
  ...
  const callback = (info) => {
    ...
    if (state.role === "GUEST") {
      alert(
        "Your access request has been sent. A super admin will review and grant approval shortly.",
      );
  ...
  if (state.role === "SUPER_ADMIN") {
    (async () => {
      try {
        const res = await runMutation("users:registerUserToAccounts", {
          args: [ids, n],
        });
        Promise.resolve().then(() => {
          const _cb = callback;
    ...
    (async () => {
      try {
        const res = await runMutation("users:requestAccountAccess", {
          email: state.userEmail,
          accountIds: ids,
          nickname: state.userNickname,
        });
...
window.openAccountSwitcher = function () {
  ...
  state.availableAccounts.forEach((acc) => {
    ...
    if (state.role === "SUPER_ADMIN")
      b.oncontextmenu = (e) => showContextMenu(e, "account", acc.id);
  ...
  document
    .getElementById("add-account-btn-trigger")
    .classList.toggle("hidden", state.role !== "SUPER_ADMIN");
...
window.submitNewAccount = function () {
  ...
  (async () => {
    try {
      const res = await runMutation("accounts:createAccount", {
        callerEmail: state.userEmail,
        accountName: n,
      });
      Promise.resolve().then(() => {
        const _cb = (info) => {
          state.availableAccounts = info.accounts;
          document
            .getElementById("account-creator-modal")
            .classList.add("hidden");
...
window.toggleManageMode = function () {
  ...
  b.className = state.isManage
    ? "bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-white text-[10px] font-black tracking-widest px-4 py-2 rounded-full uppercase"
    : "bg-primary-600 hover:bg-primary-700 text-white text-[10px] font-black tracking-widest px-4 py-2 rounded-full shadow-md uppercase";
  document
    .getElementById("add-cat-btn")
    ?.classList.toggle("hidden", !state.isManage);
  renderCategories();
  renderContent();
};
window.renderCategories = function () {
  ...
  state.currentAccount.categories.forEach((cat) => {
    ...
    if (cat.id !== "HOME" && state.isManage)
      d.oncontextmenu = (e) => showContextMenu(e, "cat", cat.id);
    l.appendChild(d);
  });
};
window.saveEdit = function () {
  ...
  if (!n) return;
  const obj = {
    id: state.context.id || Date.now().toString(),
    name: n,
    title: n,
    catId: targetCat || state.currentCat, // Use selected category or fallback
    url: u,
    iconType: i || "🔗",
  };
  ...
  (async () => {
    try {
      const res = await runMutation("accounts:saveAccountData", {
        accountId: state.currentAccount?.id,
        type: undefined,
        item: {
          id: accountId,
          name: state.context.type,
          title: state.context.type,
          url: obj,
          iconType: null,
          catId: null,
        },
      ...
      Promise.resolve().then(() => {
        const _cb = (d) => {
          if (state.context.type === "account") {
            ...
            if (
              state.currentAccount &&
              state.currentAccount.id === state.context.id
            ) {
...
window.deleteItemAction = function () {
  if (state.context.type === "account") {
    if (confirm(`Delete ${state.context.id}?`))
      (async () => {
        try {
          const res = await runMutation("accounts:deleteAccount", {
            callerEmail: state.userEmail,
            accountId: state.context.id,
          });
          Promise.resolve().then(() => {
            const _cb = (i) => {
    ...
    (async () => {
      try {
        const res = await runMutation("accounts:deleteAccountItem", {
          ...
          itemId: state.currentAccount.id,
        });
        Promise.resolve().then(() => {
          const _cb = (d) => {
            state.currentAccount = d;
...
window.renderAnnouncements = function () {
  ...
  if (
    !state.currentAccount ||
    !state.currentAccount.announcements ||
    state.currentAccount.announcements.length === 0
  ) {
    c.innerHTML =
      '<div class="flex h-full items-center justify-center text-slate-400 text-xs italic">No announcements</div>';
    return;
  }
  ...
  const mk = (a) => {
    ...
    if (state.isManage) {
      const parts = [];
      parts.push(
        `<button onclick="event.stopPropagation(); window.togglePinAction('${a.id}')" class="w-6 h-6 rounded-full bg-slate-100 text-slate-400 hover:text-yellow-500 flex items-center justify-center shadow-md"><span class="material-icons text-[14px]">push_pin</span></button>`,
      );
      parts.push(
        `<button onclick="event.stopPropagation(); window.deleteAnnAction('${a.id}')" class="w-6 h-6 rounded-full bg-red-500 text-white flex items-center justify-center shadow-lg"><span class="material-icons text-[14px]">delete</span></button>`,
      );
      const canEdit =
        state.role === "SUPER_ADMIN" ||
        (state.userEmail &&
          a.sender &&
          a.sender.toLowerCase() === state.userEmail.toLowerCase());
      if (canEdit)
        parts.unshift(
          `<button onclick="event.stopPropagation(); window.editAnnAction('${a.id}')" class="w-6 h-6 rounded-full bg-emerald-500 text-white flex items-center justify-center shadow-md"><span class="material-icons text-[14px]">edit</span></button>`,
        );
      mb = `<div class="absolute top-2 right-2 flex gap-1 z-20">${parts.join("")}</div>`;
    }
    let pi = a.isPinned
      ? `<span class="absolute -top-3 -right-2 bg-yellow-400 text-yellow-900 w-6 h-6 rounded-full flex items-center justify-center shadow-md"><span class="material-icons text-sm">push_pin</span></span>`
      : "";
    let imgHtml = "";
    ...
    const detectedLink =
      a.linkUrl ||
      (a.message.match(urlRegex) ? a.message.match(urlRegex)[0] : null);
    let linkIndicator = "";
    let ctxFn = "";
    if (detectedLink) {
  ...
  if (p.length > 0) {
    ...
    if (p.length > MAX_PIN_DISPLAY)
      newHTML += `<div class="h-px bg-slate-200 dark:bg-slate-800 my-4 mx-2"></div>`;
  }
...
window.deleteAnnAction = function (id) {
  if (confirm("Delete announcement?")) {
    ...
    (async () => {
      try {
        const res = await runMutation("unknown:deleteAccountAnnouncement", {
          args: [accountId, id],
        });
        Promise.resolve().then(() => {
          const _cb = (d) => {
...
window.editAnnAction = function (id) {
  ...
  const ann = (state.currentAccount.announcements || []).find(
    (a) => a.id === id || a.globalId === id,
  );
  if (!ann) return alert("Announcement not found");
  state.currentEditingAnn = {
    id: ann.id,
    globalId: ann.globalId || null,
    accountId: state.currentAccount.id,
  };
  document.getElementById("ann-message").value = ann.message || "";
  document.getElementById("ann-severity").value = ann.severity || "info";
  document.getElementById("ann-sender").value =
    ann.sender || state.userNickname || state.userEmail || "";
  document.getElementById("ann-link").value = ann.linkUrl || "";
  const img = ann.imageUrl || ann.imageBase64;
  if (img) {
    state.currentImageBase64 = img;
    document.getElementById("ann-image-preview").src = img;
    document
      .getElementById("ann-image-preview-container")
...
window.openPinnedAnnouncements = function () {
  ...
  list.innerHTML = "";
  const allPinned = (state.currentAccount.announcements || [])
    .filter((a) => a.isPinned)
    .reverse();
  if (!allPinned.length)
    list.innerHTML =
      '<div class="text-sm text-slate-500">No pinned broadcasts.</div>';
  allPinned.forEach((a) => {
...
window.renderPinnedAnnouncements = function () {
  ...
  const wasOpen = !document
    .getElementById("pinned-ann-modal")
...
window.togglePinAction = function (id, cb) {
  ...
  (async () => {
    try {
      const res = await runMutation("unknown:toggleAnnouncementPin", {
        args: [accId, id],
      });
      Promise.resolve().then(() => {
        const _cb = (d) => {
...
window.releaseAnnouncement = function () {
  ...
  if (state.currentEditingAnn) {
    ...
    (async () => {
      try {
        const res = await runMutation("unknown:editAccountAnnouncement", {
          args: [
            accId,
            state.currentEditingAnn.id,
            m,
            s,
  ...
  (async () => {
    try {
      const res = await runMutation("unknown:postAccountAnnouncement", {
        args: [target, m, s, snd, state.currentImageBase64, link],
      });
      Promise.resolve().then(() => {
        const _cb = (d) => {
          try {
              ...
              try {
                pollWorkspaceUpdates(() => {
                  try {
                    ...
                    console.error("renderAnnouncements after poll failed", e);
                  }
                  try {
                    closeModals();
                  } catch (e) {
                    console.error("closeModals after poll failed", e);
                  }
                });
              } catch (e) {
            ...
            alert(
              "An error occurred while processing the announcement response.",
            );
          } finally {
            window.setBtnLoading("ann-post-btn", false);
          }
...
window.openModal = function () {
  ...
  document.getElementById("modal-title").innerText = state.context.id
    ? "Modify"
    : "New";
  document.getElementById("url-field").classList.toggle("hidden", ic || isAcc);
  document
    .getElementById("icon-type-field")
    .classList.toggle("hidden", ic || isAcc);
  // Reset Fields
  ...
  if (state.currentAccount && state.currentAccount.categories) {
    state.currentAccount.categories.forEach((cat) => {
      if (cat.id !== "HOME") {
        // Don't allow saving to Home dashboard directly as per tool logic
  ...
  document
    .getElementById("category-select-container")
    .classList.toggle("hidden", ic || isAcc);
  if (state.context.id) {
    let it;
    if (ic)
      it = state.currentAccount.categories.find(
        (c) => c.id === state.context.id,
      );
    else if (isAcc)
      it = state.availableAccounts.find((a) => a.id === state.context.id);
...
window.closeModals = function () {
  ...
  const preview = document.getElementById("ann-image-preview-container");
...
window.openBroadcaster = function (forceServer) {
  ...
  if (!state.currentEditingAnn) {
    ...
    document.getElementById("ann-sender").value =
      state.userNickname || state.userEmail || "";
  ...
  if (postBtn) {
    postBtn.innerText = state.currentEditingAnn
      ? "Update Announcement"
      : "Post Announcement";
  }
  ...
  if (sel) {
    sel.innerHTML = "";
    const currLabel = state.currentAccount
      ? state.currentAccount.name ||
        state.currentAccount.title ||
        state.currentAccount.id
      : "Current Account";
    ...
    if (state.role === "SUPER_ADMIN" && forceServer === true) {
      ...
      optAll.value = "ALL";
      optAll.innerText = "All Accounts (Global Announcement)";
    ...
    try {
      if (state.currentAccount && state.currentAccount.id)
        sel.value = state.currentAccount.id;
    } catch (e) {}
  ...
  // Prefill sender
  const sndEl = document.getElementById("ann-sender");
  if (sndEl && !state.currentEditingAnn)
    sndEl.value = state.userNickname || state.userEmail || "";
  // If forced server, select ALL
  if (forceServer && document.getElementById("ann-target"))
...
window.openImageZoom = function (u) {
  ...
  if (!lightbox) {
    ...
    lightbox.className =
      "fixed inset-0 z-[9999] bg-black/90 hidden items-center justify-center p-4 cursor-zoom-out";
...
window.addPersonalTaskAction = function () {
  ...
  (async () => {
    try {
      const res = await runMutation("tasks:addPersonalTask", {
        email: state.userEmail,
        taskText: t,
      });
      Promise.resolve().then(() => {
...
window.toggleTaskAction = function (id) {
  ...
  if (t) {
    ...
    (async () => {
      try {
        const res = await runMutation("tasks:togglePersonalTask", {
          email: state.userEmail,
          taskId: id,
        });
        Promise.resolve().then(() => {
          const _cb = (l) => (state.checklist = l || []);
          if (typeof _cb === "function") _cb(res);
        });
      } catch (err) {
        console.error(err);
...
window.deleteTaskAction = function (id) {
  ...
  (async () => {
    try {
      const res = await runMutation("tasks:deletePersonalTask", {
        email: state.userEmail,
        taskId: id,
      });
      Promise.resolve().then(() => {
...
window.closeTaskAssignmentModal = function () {
  ...
  if (
    state.bulkTaskQueue.length > 0 &&
    state.currentTaskTarget &&
    state.currentTaskTarget.email
  ) {
...
window.setDashboardMode = function (mode) {
  ...
  document
    .getElementById("note-tab-personal")
    .classList.toggle("active", mode === "PERSONAL");
  document
    .getElementById("note-tab-team")
...
window.saveDashNote = function () {
  ...
  if (t || b) {
    ...
    (async () => {
      try {
        const res = await runMutation("notes:saveNote", {
          email: state.userEmail,
          noteId: null,
          title: t,
          content: b,
          scope: state.notesMode,
          accountId: aid,
          nickname: state.userNickname,
        });
        Promise.resolve().then(() => {
...
window.renderFullScreenNotes = function () {
  ...
  if (state.notesMode === "TEAM") {
    if (!state.currentAccount) {
      grid.innerHTML =
        '<div class="col-span-full text-center text-slate-500 mt-20 font-black uppercase tracking-widest text-[10px]">Join a workspace to view team notes.</div>';
      return;
    }
  ...
  if (list.length === 0) {
    grid.innerHTML =
      '<div class="col-span-full text-center text-slate-500 mt-20 flex flex-col items-center"><span class="material-icons text-6xl opacity-20 mb-4">note_add</span><span class="text-[10px] font-black uppercase tracking-widest opacity-40">Notes you add appear here</span></div>';
    return;
  }
  grid.innerHTML = list
    .map((n) => {
      const auth =
        state.notesMode === "TEAM" && n.author
...
window.openNoteEditor = function (id) {
  ...
  const l =
    state.notesMode === "TEAM"
      ? state.currentAccount
        ? state.currentAccount.notes
        : []
      : state.notes;
  const n = id
    ? (l || []).find((x) => x.id === id)
...
window.deleteNoteFromEditor = function () {
  ...
  if (state.notesMode === "TEAM")
    state.currentAccount.notes = state.currentAccount.notes.filter(
      (n) => n.id !== id,
    );
  ...
  (async () => {
    try {
      const res = await runMutation("notes:deleteNote", {
        email: state.userEmail,
        noteId: id,
        scope: state.notesMode,
        accountId: aid,
...
window.toggleNotesMode = function () {
  ...
  document
    .getElementById("note-tab-personal")
    .classList.toggle("active", state.notesMode === "PERSONAL");
  document
    .getElementById("note-tab-team")
    .classList.toggle("active", state.notesMode === "TEAM");
  renderNotes();
  // Update Dashboard if open
  if (
    !document
      .getElementById("notes-dashboard-modal")
      .classList.contains("hidden")
  ) {
    updateDashboardModeButtons();
    renderFullScreenNotes();
...
window.renderNotes = function () {
  ...
  if (state.notesMode === "TEAM") {
    if (!state.currentAccount) {
      c.innerHTML =
        '<div class="text-center text-xs opacity-50 p-4">Join workspace for team notes</div>';
      return;
    }
    list = state.currentAccount.notes || [];
  } else {
  ...
  if (list.length === 0) {
    newHTML =
      '<div class="text-center text-xs opacity-50 p-4">No notes yet</div>';
  } else {
    newHTML = list
      .map((n) => {
        let auth =
          state.notesMode === "TEAM" && n.author
            ? `<span class="text-[9px] font-bold text-slate-400 dark:text-slate-400 uppercase block mb-1">${n.author}</span>`
            : "";
...
window.loadOrgChartData = function (isRefresh) {
  ...
  if (!isRefresh) {
    main.innerHTML =
      '<div class="flex flex-col items-center justify-center h-full text-slate-500"><span class="material-icons text-4xl animate-spin mb-4">sync</span><span class="text-xs font-bold uppercase tracking-widest">Loading Roster...</span></div>';
  }
...
function renderOrgChart(data) {
  ...
  if (!data.departments || data.departments.length === 0) {
    main.innerHTML =
      '<div class="flex flex-col items-center justify-center h-full text-slate-500"><span class="material-icons text-4xl mb-4">person_off</span><span class="text-xs font-bold uppercase tracking-widest">No data found. Initialize the Roster sheet first.</span></div>';
    return;
  }
  ...
  document.getElementById("org-last-updated").textContent =
    "Last Updated Data: " +
    new Date().toLocaleTimeString([], {
      hour: "2-digit",
      minute: "2-digit",
...
function buildOrgNode(node) {
  ...
  const activeDot = node.isActive
    ? '<div class="org-active-dot" title="Currently Active"></div>'
    : "";
...
function getOrgRoleClass(role) {
  ...
  if (r === "manager" || r === "learning & development manager")
    return "org-rc-m";
...
function showOrgTip(e, node) {
  ...
  const lobs = node.lob
    ? node.lob
        ...
        .filter(Boolean)
    : [];
  const lobHtml = lobs.length
    ? lobs.map((l) => `<span class="org-ttp">${escHtml(l)}</span>`).join("")
...
window.openReminderCreator = function () {
  ...
  document
    .getElementById("reminder-image-preview-container")
    .classList.add("hidden");
...
window.sendReminder = function () {
  ...
  (async () => {
    try {
      const res = await runMutation("reminders:postReminder", {
        callerEmail: state.userEmail,
        targetAccount: t,
        message: m,
        imageUrl: state.currentReminderImageBase64,
        sender: s,
        durationHours: d,
        scheduledTime: sc,
...
window.deleteReminder = function (id) {
  ...
  (async () => {
    try {
      const res = await runMutation("reminders:deleteReminder", {
        callerEmail: state.userEmail,
        reminderId: id,
      });
      Promise.resolve().then(() => {
...
window.fetchWeatherRisk = async function () {
  ...
  try {
    ...
    const r = await fetch(
      `https://api.open-meteo.com/v1/forecast?latitude=${c.lat}&longitude=${c.lon}&hourly=rain,showers,wind_speed_10m,cloud_cover&current=rain,showers,cloud_cover,wind_speed_10m&timezone=America%2FNew_York&forecast_days=2`,
    );
    ...
    localStorage.setItem(
      "zeus_weather_cache",
      JSON.stringify(state.weatherRisk),
    );
  } catch (e) {
    if (!state.weatherRisk) {
      state.weatherRisk = {
...
window.sendQuestionToAI = function () {
  ...
  (async () => {
    try {
      const res = await runMutation("unknown:askAIBot", { args: [question] });
      Promise.resolve().then(() => {
        const _cb = (response) => {
