/**
 * app.js - WF Zeus Transition Frontend
 * Adapted from JS.html. Replace google.script.run calls with Convex calls.
 * Import the Convex API functions from convex-client.js and _generated/api.js
 */
import { convex, runQuery, runMutation } from './convex-client.js';

// NOTE: Replace (async () => { try { const res = await runMutation("unknown:methodName", { args: [args] }); if (typeof cb === 'function') { cb(res); } } catch(err) { console.error(err); } })() with:
//   convex.query(api.module.functionName, args).then(cb)   -- for queries
//   convex.mutation(api.module.functionName, args).then(cb) -- for mutations
  ...
  let state = {
    ...
    lastEmojis: JSON.parse(localStorage.getItem('zeus_last_emojis') || '["ðŸ‘", "â¤ï¸", "ðŸ˜‚"]'),
  ...
  window.unassignAccount = function (accId) {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("users:requestAccountRemoval", { email: state.userEmail, accountId: accId }); if (typeof () => {
  ...
  window.onload = function () {
    ...
    window.updateActivity = function () {
      ...
      if (s.status !== 'OFFLINE' && timeDiff > awayTime) {
        ...
        // Sync to server
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
        (async () => { try { const res = await runMutation("users:updateUserAuxStatus", { email: state.userEmail, auxStatus: 'ONLINE' }); } catch(err) { console.error(err); } })();
      }
  ...
  window.pollPersonalUpdates = function (onComplete) {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runQuery("users:getSessionInfo", {}); if (typeof data => {
          ...
          if (n.type === 'access') {
            ...
            if (n.approved) {
              ...
              if (n.requestType === 'REMOVAL' && state.currentAccount && state.currentAccount.id === n.accountId) {
                alert(`Your removal request for workspace ${accName} has been approved.`);
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
                (async () => { try { const res = await runQuery("tasks:fetchPersonalUpdates", { email: state.userEmail }); if (typeof function (info) {
                  state.userAccounts = info.userAccounts || [];
                  state.role = info.role;
                  if (state.userAccounts.length > 0) {
                    window.loadAccount(state.userAccounts[0].id);
      ...
      if (onComplete) onComplete();
    }(res); } } catch(err) { console.error(err); if (typeof () => { if (onComplete) onComplete(); } === 'function') { () => { if (onComplete) onComplete(); }(err); } else { alert(err.message || String(err)); } } })();
  };
  ...
  window.pollWorkspaceUpdates = function (onComplete) {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runQuery("accounts:getAccountData", { accountId: state.currentAccount.id }); if (typeof data => {
  ...
  window.dispatchBulkTasks = function () {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("tasks:bulkAssignTasks", { callerEmail: state.userEmail, targetType: state.bulkTargetType, targetId: targetId, tasks: state.bulkTaskQueue, senderNickname: state.userNickname }); if (typeof msg => {
      alert(msg);
      state.bulkTaskQueue = [];
      renderTaskQueue();
  ...
  window.fetchLiveStatus = function () {
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runQuery("status:getLiveStatus", {}); if (typeof renderLiveStatus === 'function') { renderLiveStatus(res); } } catch(err) { console.error(err); } })();
  };
  ...
  window.fetchTaskHistory = function () {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runQuery("tasks:getTaskHistory", { callerEmail: state.userEmail, filterAccount: acc, filterUser: usr, dateFrom: dFrom, dateTo: dTo }); if (typeof renderTaskHistory === 'function') { renderTaskHistory(res); } } catch(err) { console.error(err); } })();
  };
  ...
  window.sendFeedback = function () {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("feedback:submitFeedback", { email: state.userEmail, nickname: state.userNickname, message: msg }); if (typeof () => {
  ...
  window.loadFeedbacks = function () {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("function toLocaleString() { [native code] }", { args: [[], { timeZone: state.systemTimeZone }] }); if (typeof feedbacks => {
      ...
      list.innerHTML = feedbacks.map(f => `
            <div class="bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700">
                <div class="flex justify-between items-start mb-2">
                    ...
                    <span class="text-[8px] font-bold text-slate-400 uppercase">${new Date(f.timestamp === 'function') { feedbacks => {
      if (feedbacks.length === 0) {
        list.innerHTML = '<div class="text-center text-slate-400 text-xs py-10 font-bold uppercase tracking-widest">No feedbacks yet</div>';
        return;
      }
      list.innerHTML = feedbacks.map(f => `
            <div class="bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700">
                <div class="flex justify-between items-start mb-2">
                    <span class="text-[10px] font-black uppercase text-primary-600 tracking-widest">${f.sender}</span>
                    <span class="text-[8px] font-bold text-slate-400 uppercase">${new Date(f.timestamp(res); } } catch(err) { console.error(err); } })()}</span>
                </div>
  ...
  function initSession() {
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runQuery("users:getSessionInfo", {}); if (typeof function (info) {
      state.role = info.role;
  ...
  window.toggleMaintenanceMode = function () {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("users:toggleMaintenanceMode", {}); if (typeof status => {
  ...
  window.loadAccount = async function (accountId) {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("unknown:includes", { args: ['walmart'] }); if (typeof function (data) {
      ...
      const isWalmart = data.name.toLowerCase( === 'function') { function (data) {
      if (!data) { window.showAccountSelector(); return; }
      state.currentAccount = data;
      state.view = 'PORTAL';
      state.isManage = false;
      ...
      if (srv) srv.classList.add('hidden');
      // Org Chart Button Visibility (Walmart Only)
      const isWalmart = data.name.toLowerCase((res); } } catch(err) { console.error(err); } })();
  ...
  window.fetchRegistry = function () {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("unknown:getUsersRegistry", {}); if (typeof function (registry) {
  ...
  window.unregisterAction = function (aid, email) {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
      (async () => { try { const res = await runMutation("accounts:unregisterUser", { callerEmail: state.userEmail, accountId: aid, email: email.trim( }); if (typeof r => renderAdminRegistry(r) === 'function') { r => renderAdminRegistry(r)(res); } } catch(err) { console.error(err); } })().toLowerCase());
    }
  };
  // Access request UI
  ...
  window.fetchAccessRequests = function () {
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runQuery("users:getAccessRequests", {}); if (typeof renderAccessRequests === 'function') { renderAccessRequests(res); } } catch(err) { console.error(err); } })();
  };
  ...
  window.approveRequest = function (requestId) {
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("users:approveAccountAccess", { callerEmail: state.userEmail, requestId: requestId }); if (typeof () => {
      fetchAccessRequests();
      fetchRegistry();
    } === 'function') { () => {
      fetchAccessRequests();
  ...
  window.rejectRequest = function (requestId) {
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("users:rejectAccountAccess", { callerEmail: state.userEmail, requestId: requestId }); if (typeof () => {
      fetchAccessRequests();
    } === 'function') { () => {
      fetchAccessRequests();
    }(res); } } catch(err) { console.error(err); } })();
  ...
  window.submitRegistration = function () {
    ...
    if (state.role === 'SUPER_ADMIN') {
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
      (async () => { try { const res = await runMutation("users:registerUserToAccounts", { args: [ids, n] }); if (typeof callback === 'function') { callback(res); } } catch(err) { console.error(err); } })();
    } else {
      // send approval request instead
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
      (async () => { try { const res = await runMutation("users:requestAccountAccess", { email: state.userEmail, accountIds: ids, nickname: state.userNickname }); if (typeof callback === 'function') { callback(res); } } catch(err) { console.error(err); } })();
    }
  };
  ...
  window.submitNewAccount = function () {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("accounts:createAccount", { callerEmail: state.userEmail, accountName: n }); if (typeof info => {
      state.availableAccounts = info.accounts;
  ...
  window.saveEdit = function () {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("accounts:saveAccountData", { accountId: state.currentAccount?.id, type: undefined, item: { id: accountId, name: state.context.type, title: state.context.type, url: obj, iconType: null, catId: null } }); if (typeof d => {
  ...
  window.deleteItemAction = function () {
    if (state.context.type === 'account') {
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
      if (confirm(`Delete ${state.context.id}?`)) (async () => { try { const res = await runMutation("accounts:deleteAccount", { callerEmail: state.userEmail, accountId: state.context.id }); if (typeof i => {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    if (confirm("Remove item?")) (async () => { try { const res = await runMutation("accounts:deleteAccountItem", { accountId: state.currentAccount?.id, type: state.context.type, itemId: state.currentAccount.id }); if (typeof d => {
      state.currentAccount = d;
      renderCategories();
      renderContent();
    } === 'function') { d => {
  ...
  window.deleteAnnAction = function (id) {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
      (async () => { try { const res = await runMutation("unknown:deleteAccountAnnouncement", { args: [accountId, id] }); if (typeof d => {
        state.currentAccount = d;
        renderAnnouncements();
      } === 'function') { d => {
        state.currentAccount = d;
  ...
  window.openPinnedAnnouncements = function () {
    ...
    allPinned.forEach(a => {
      ...
      el.innerHTML = `<div class="flex justify-between items-start"><div><div class="text-[10px] font-black">${a.sender} â€¢ ${new Date(a.timestamp).toLocaleString()}</div><div class="mt-2 text-sm">${parseMarkdown(a.message)}</div></div><div class="ml-4 text-right"><button onclick="event.stopPropagation(); window.togglePinAction('${a.id}', renderPinnedAnnouncements);" class="text-xs px-2 py-1 bg-slate-100 rounded">Unpin</button></div></div>`;
      list.appendChild(el);
    });
  ...
  window.togglePinAction = function (id, cb) {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("unknown:toggleAnnouncementPin", { args: [accId, id] }); if (typeof d => {
      try {
        state.currentAccount = d;
        renderAnnouncements();
    ...
    }(res); } } catch(err) { console.error(err); if (typeof err => {
      console.error('Failed to toggle pin', err);
      alert('Failed to toggle pin (server error)');
    } === 'function') { err => {
      console.error('Failed to toggle pin', err);
      alert('Failed to toggle pin (server error)');
    }(err); } else { alert(err.message || String(err)); } } })();
  };
  window.releaseAnnouncement = function () {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
      (async () => { try { const res = await runMutation("unknown:editAccountAnnouncement", { args: [accId, state.currentEditingAnn.id, m, s, state.currentImageBase64, link] }); if (typeof d => {
        if (d && d.status === 'error') {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("unknown:postAccountAnnouncement", { args: [target, m, s, snd, state.currentImageBase64, link] }); if (typeof d => {
      try {
  ...
  window.addPersonalTaskAction = function () {
    ...
    if (state.currentCat === 'HOME') updateChecklistUI();
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("tasks:addPersonalTask", { email: state.userEmail, taskText: t }); if (typeof l => {
      state.checklist = l || [];
      if (state.currentCat === 'HOME') updateChecklistUI();
    } === 'function') { l => {
      state.checklist = l || [];
  ...
  window.toggleTaskAction = function (id) {
    ...
    if (t) {
      ...
      if (state.currentCat === 'HOME') updateChecklistUI();
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
      (async () => { try { const res = await runMutation("tasks:togglePersonalTask", { email: state.userEmail, taskId: id }); if (typeof l => state.checklist = l || [] === 'function') { l => state.checklist = l || [](res); } } catch(err) { console.error(err); } })();
    }
  };
  window.deleteTaskAction = function (id) {
    ...
    if (state.currentCat === 'HOME') updateChecklistUI();
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("tasks:deletePersonalTask", { email: state.userEmail, taskId: id }); if (typeof l => {
      state.checklist = l || [];
      if (state.currentCat === 'HOME') updateChecklistUI();
    } === 'function') { l => {
      state.checklist = l || [];
  ...
  window.dispatchTasksToUser = function (userEmail) {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("tasks:bulkAssignTasks", { callerEmail: state.userEmail, targetType: 'USER', targetId: userEmail, tasks: taskList, senderNickname: state.userNickname }); if (typeof () => {
  ...
  window.saveDashNote = function () {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
      (async () => { try { const res = await runMutation("notes:saveNote", { email: state.userEmail, noteId: null, title: t, content: b, scope: state.notesMode, accountId: aid, nickname: state.userNickname }); if (typeof n => {
  ...
  window.saveNoteFromEditor = function () {
    ...
    const aid = state.currentAccount ? state.currentAccount.id : null;
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("notes:saveNote", { email: state.userEmail, noteId: state.currentNoteId, title: t, content: b, scope: state.notesMode, accountId: aid, nickname: state.userNickname }); if (typeof nx => {
  ...
  window.deleteNoteFromEditor = function () {
    ...
    const aid = state.currentAccount ? state.currentAccount.id : null;
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("notes:deleteNote", { email: state.userEmail, noteId: id, scope: state.notesMode, accountId: aid }); if (typeof n => {
      if (state.notesMode === 'TEAM') state.currentAccount.notes = n;
      else state.notes = n;
      renderNotes();
      renderFullScreenNotes();
  ...
  window.saveNoteAction = function () {
    ...
    const aid = state.currentAccount ? state.currentAccount.id : null;
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("notes:saveNote", { email: state.userEmail, noteId: null, title: tv, content: bv, scope: state.notesMode, accountId: aid, nickname: state.userNickname }); if (typeof n => {
      if (state.notesMode === 'TEAM') state.currentAccount.notes = n;
      else state.notes = n;
      renderNotes();
    } === 'function') { n => {
  ...
  window.loadOrgChartData = function (isRefresh) {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    fetch('/orgchart.json').then(r=>r.json()).then(data => { if(data => {
  ...
  function showOrgTip(e, node) {
    ...
    const lobHtml = lobs.length ? lobs.map(l => `<span class="org-ttp">${escHtml(l)}</span>`).join('') : '<span class="text-slate-600 italic">â€”</span>';
    tti.innerHTML = `
    <div class="org-tth">
    ...
    <div class="org-ttr">
      ...
      <div class="org-ttv">${node.email ? `<a href="mailto:${node.email}" class="text-primary-600 dark:text-primary-400 hover:underline">${escHtml(node.email)}</a>` : '<span class="italic">â€”</span>'}</div>
    </div>
    <div class="org-ttr">
    ...
    <div class="org-ttr">
      ...
      <div class="org-ttv">${escHtml(node.scheduleHours || 'â€”')}</div>
    </div>
    <div class="org-ttr">
      <span class="material-icons org-ttic text-sm">calendar_today</span>
      <div class="org-ttv">${escHtml(node.scheduleDays || 'â€”')}</div>
    </div>
  `;
    tt.classList.add('on');
    tt.style.opacity = '1';
  ...
  window.sendReminder = function () {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("reminders:postReminder", { callerEmail: state.userEmail, targetAccount: t, message: m, imageUrl: state.currentReminderImageBase64, sender: s, durationHours: d, scheduledTime: sc, isRecurring: isRecurring, recurrenceRule: rec }); if (typeof () => {
  ...
  window.deleteReminder = function (id) {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("reminders:deleteReminder", { callerEmail: state.userEmail, reminderId: id }); if (typeof () => {
      if (state.currentAccount) window.loadAccount(state.currentAccount.id);
    } === 'function') { () => {
      if (state.currentAccount) window.loadAccount(state.currentAccount.id);
    }(res); } } catch(err) { console.error(err); } })();
  ...
  window.setAuxStatus = function (status) {
    ...
    // TODO-CONVEX: Replace google.script.run call below with Convex mutation/query
    (async () => { try { const res = await runMutation("users:updateUserAuxStatus", { email: state.userEmail, auxStatus: status }); if (typeof () => {
      checkOnlineStatus();
    } === 'function') { () => {
      checkOnlineStatus();
    }(res); } } catch(err) { console.error(err); } })();
